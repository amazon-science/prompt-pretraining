...
class GeneralizedSemSegEvaluator(SemSegEvaluator):
    ...
    def process(self, inputs, outputs):
        ...
        for input, output in zip(inputs, outputs):
            ...
            self.instance_miou.append((input['file_name'], miou))
    ...
    def evaluate(self):
        ...
        * Pixel Accuracy (pACC)
        ...
        if self._distributed:
            ...
            self.instance_miou = all_gather(self.instance_miou)
            self.instance_miou = list(itertools.chain(*self.instance_miou))
            if not is_main_process():
                return
            if not os.path.exists(self._output_dir):
                os.mkdir(self._output_dir)
            file_path = os.path.join(self._output_dir, "instance_miou.pth")
            with PathManager.open(file_path, "wb") as f:
                torch.save(self.instance_miou, f)
